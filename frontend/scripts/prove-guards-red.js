...
const CASES = [
  ...
  { suite: 'scoreBands',
    ...
    file: 'lib/scoreBands.js',
    mutate: (s) => s.replace("if (score === null || score === undefined || score === '') return null;", '') },
  { suite: 'scoreBands',
    test: 'keeps the number alongside the word',
    file: 'lib/scoreBands.js',
    mutate: (s) => s.replace('return `${band.label} · ${Math.round(Number(score) * 100)}%`;',
      'return band.label;') },
  { suite: 'statusWords',
    test: 'does not claim an employer saw it before they did',
    file: 'lib/statusWords.js',
    mutate: (s) => s.replace("submitted: { label: 'Waiting for the company'", "submitted: { label: 'Submitted'") },
  { suite: 'statusWords',
    test: 'humanises a status nobody wrote a line for',
    file: 'lib/statusWords.js',
    mutate: (s) => s.replace("return String(status).replace(/[_-]+/g, ' ').replace(/^\\w/, (c) => c.toUpperCase());", 'return status;') },
  { suite: 'statusWords',
    test: 'the pipeline columns come from statusWord',
    file: 'pages/applications.js',
    mutate: (s) => s.replace("{ key: 'phone_screen', label: statusWord('phone_screen') },",
      "{ key: 'phone_screen', label: 'Phone Screen' },") },
  { suite: 'statusWords',
    test: 'the nav says plain words',
    file: 'components/DashboardLayout.js',
    mutate: (s) => s.replace("label: 'Ready to send',", "label: 'Apply Queue',") },
